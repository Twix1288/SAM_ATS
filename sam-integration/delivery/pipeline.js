 ...
 *   2. ATTACHMENT  the full designed Snapshot, appended via candidate.uploadFile
...
export async function deliverSnapshot(ctx) {
  ...
  let attachment = null;
  try {
    attachment = await uploadSnapshotFile({ candidateId, filename, bytes: pdf, deliveryId });
    record('attach', true, `${filename} appended via ${attachment.via} · ${attachment.bytes} bytes`);
  } catch (err) {
